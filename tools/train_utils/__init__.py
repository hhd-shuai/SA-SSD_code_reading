...
def batch_processor(model, data):
    ...
    outputs = dict(
        loss=loss, log_vars=log_vars, num_samples=len(data['img'].data)) #{'loss': tensor(122.1386, device='cuda:0', grad_fn=<AddBackward0>), 'log_vars': OrderedDict([('aux_loss_cls', 0.3507128357887268), ('aux_loss_reg', 0.9529203772544861), ('rpn_loc_loss', 8.606904983520508), ('rpn_cls_loss', 42.82087707519531), ('rpn_dir_loss', 0.24040904641151428), ('loss_cls', 69.16682434082031), ('loss', 122.1386489868164)]), 'num_samples': 2}
...
def train_model(model, optimizer, train_loader, lr_scheduler, optim_cfg, # optim_cfg: {'type': 'adam_onecycle', 'lr': 0.003, 'weight_decay': 0.01, 'grad_clip': {'max_norm': 10, 'norm_type': 2}}
                start_epoch, total_epochs, start_iter, rank, logger, ckpt_save_dir, # start_epoch: 0, total_epochs: 20, start_iter: 0, rank: 0, ckpt_save_dir: './work_dir/origin'
                lr_warmup_scheduler=None, ckpt_save_interval=1, max_ckpt_save_num=50, log_interval=20): # lr_warmup_scheduler: None, ckpt_save_interval: 10, max_ckpt_save_num: 10, log_interval: 20
