...
# 计算相机视场范围内LiDAR点云的个数
# 根据雷达外参数把点云投在相机坐标系下，然后滤除相机视场外的点，
# 计算剩下点云的个数，把结果更新到annos["num_points_in_gt"]中。
def _calculate_num_points_in_gt(data_path, infos, relative_path, remove_outside=True, num_features=4): #infos: 3712
    for info in infos:
        if relative_path:
            v_path = str(pathlib.Path(data_path) / info["velodyne_path"]) # '/home/b516/hhd_shuai/vscode_file/SA-SSD_code_reading/KITTI/training/velodyne/000000.bin'
        else:
            v_path = info["velodyne_path"]
        points_v = np.fromfile(
            v_path, dtype=np.float32, count=-1).reshape([-1, num_features]) # ndarray：（115384，4）
        rect = info['calib/R0_rect'] #4*4
        Trv2c = info['calib/Tr_velo_to_cam'] #4*4
        P2 = info['calib/P2'] #4*4
        if remove_outside:
...
def create_kitti_info_file(data_path,
    ...
    # 计算相机视场范围内LiDAR点云的个数。点云总数会存放在字典变量kitti_infos_train中。结果将存放在pkl类型文件
...
def create_reduced_point_cloud(data_path, #'/home/b516/hhd_shuai/vscode_file/SA-SSD_code_reading/KITTI'
...
if __name__ == '__main__':
    my_root_path = "/home/b516/hhd_shuai/vscode_file/SA-SSD_code_reading/KITTI"
