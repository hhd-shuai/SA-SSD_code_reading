...
def get_kitti_info_path(idx, # 13
                        prefix, # '/home/b516/hhd_shuai/vscode_file/SA-SSD_code_reading/KITTI'
                        info_type='image_2', # 'velodyne' 'image_2'
                        file_tail='.png', # '.bin' '.png'
                        training=True,
                        relative_path=True,
                        exist_check=True):
    img_idx_str = get_image_index_str(idx) # '000013'
    img_idx_str += file_tail # '000013.bin' or '000013.png' or '000013.txt'
    prefix = pathlib.Path(prefix)
    if training:
        file_path = pathlib.Path('training') / info_type / img_idx_str # training/velodyne/000013.bin； training/image_2/000013.png
    else:
...
def get_kitti_image_info(path, # '/home/b516/hhd_shuai/vscode_file/SA-SSD_code_reading/KITTI'
                         training=True, # True
                         label_info=True, # True
                         velodyne=False, # True
                         calib=False, # True
                         image_ids=7481, # list：3712   0，3，7，9，10，11，...
                         extend_matrix=True, # True
                         num_worker=8,
                         relative_path=True, # True
                         with_imageshape=True): # True
    # image_infos = []
    ...
    def map_func(idx):
        image_info = {'image_idx': idx, 'pointcloud_num_features': 4} # {'image_idx': 13, 'pointcloud_num_features': 4}
        annotations = None
        if velodyne:
            image_info['velodyne_path'] = get_velodyne_path(
                idx, path, training, relative_path) #'velodyne_path'='training/velodyne/000013.bin'
        image_info['img_path'] = get_image_path(idx, path, training,
                                                relative_path) #'training/image_2/000013.png'
        if with_imageshape:
            img_path = image_info['img_path']
            if relative_path:
                img_path = str(root_path / img_path)
            image_info['img_shape'] = np.array(
                imread(img_path).shape[:2], dtype=np.int32) #[ 375 1242]
        if label_info:
            #找到目标检测标签label的路径
            label_path = get_label_path(idx, path, training, relative_path) #'/home/b516/hhd_shuai/vscode_file/SA-SSD_code_reading/KITTI/training/label_2/000013.txt'
            if relative_path:
                # 如果是相对路径就加上前缀，得到绝对路径
        ...
        if calib:
            calib_path = get_calib_path(
                idx, path, training, relative_path=False) #'/home/b516/hhd_shuai/vscode_file/SA-SSD_code_reading/KITTI/training/calib/000013.txt'
            ...
            P3 = np.array(
                [float(info) for info in lines[3].split(' ')[1:13]]).reshape(
                    [3, 4])
            if extend_matrix:
                P0 = _extend_matrix(P0) #ndarray:(4, 4)
            ...
            R0_rect = np.array([ # [[ 0.9999239   0.00983776 -0.00744505], [-0.0098698   0.9999421  -0.00427846], [ 0.00740253  0.00435161  0.9999631 ]]
                float(info) for info in lines[4].split(' ')[1:10]
            ]).reshape([3, 3])
            if extend_matrix:
                rect_4x4 = np.zeros([4, 4], dtype=R0_rect.dtype)
                rect_4x4[3, 3] = 1.
                rect_4x4[:3, :3] = R0_rect #[[ 0.9999239   0.00983776 -0.00744505  0.        ], [-0.0098698   0.9999421  -0.00427846  0.        ], [ 0.00740253  0.00435161  0.9999631   0.        ], [ 0.          0.          0.          1.        ]]
            else:
                rect_4x4 = R0_rect
            image_info['calib/R0_rect'] = rect_4x4
            Tr_velo_to_cam = np.array([
                float(info) for info in lines[5].split(' ')[1:13]
            ]).reshape([3, 4]) # [[ 7.533745e-03 -9.999714e-01 -6.166020e-04 -4.069766e-03], [ 1.480249e-02  7.280733e-04 -9.998902e-01 -7.631618e-02], [ 9.998621e-01  7.523790e-03  1.480755e-02 -2.717806e-01]]
    ...
    with futures.ThreadPoolExecutor(num_worker) as executor:
        ...
        # 输出image_info是一个字典型变量，包含'image_idx'，pointcloud_num_features，velodyne_path，img_path，img_shape，annos。
...
def add_difficulty_to_annos(info): #
    ...
    annos = info['annos']
    dims = annos['dimensions']  # lhw format  [[ 3.47  1.45  1.56], [-1.   -1.   -1.  ]]
    bbox = annos['bbox'] # [[455.7  183.86 533.81 241.91], [574.15 164.6  600.23 189.64]]
    height = bbox[:, 3] - bbox[:, 1]  # [58.05 25.04]
    occlusion = annos['occluded']  # [ 0 -1]
    truncation = annos['truncated'] # [ 0. -1.]
    diff = []
    easy_mask = np.ones((len(dims), ), dtype=np.bool) #[ True False]
    ...
    is_moderate = np.logical_xor(easy_mask, moderate_mask) #[False  True]
    is_hard = np.logical_xor(hard_mask, moderate_mask) #[False False]
    ...
    for i in range(len(dims)):
        if is_easy[i]:
            ...
            diff.append(-1)
    annos["difficulty"] = np.array(diff, np.int32) #[0 1]
    return diff
...
def get_label_anno(label_path):
    ...
    # 在annos中，truncated和occluded用于衡量该3d目标被遮挡的程度，用于给该3d目标被检测到的难易程度打分。
