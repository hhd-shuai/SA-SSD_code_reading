...
class SSDRotateHead(nn.Module):
    ...
    def __init__(self,
                 num_class=1, # num_class=1 3D 目标检测类别，一类，car
                 num_output_filters=768,  # num_output_filters: 256
                 num_anchor_per_loc=2, # num_anchor_per_loc: 2 单元位置中Anchor的数量，如果是两个，那就是横放的Anchor和竖放的Anchor
                 use_sigmoid_cls=True, # use_sigmoid_cls: True 使用sigmoid函数用于分类
                 encode_rad_error_by_sin=True, # encode_rad_error_by_sin: True 使用sin函数计算误差角
                 use_direction_classifier=True, # use_direction_classifier: True 对方向进行分类（正对相机，背对相机）
                 box_coder='GroundBox3dCoder', # box_coder: 'GroundBox3dCoder' 有关3D框的参数
                 box_code_size=7, # box_code_size: 7 用7个参数表述一个3D框。分别是xyzhwl以及score
                 ):
        super(SSDRotateHead, self).__init__()
        num_anchor_per_loc *= num_class

        # 如果使用sigmiod，num_cls意思是每个位置的Anchor都要判别类别
        if use_sigmoid_cls:
        ...
        self._use_direction_classifier = use_direction_classifier
        # getattr 返回object的名称为name的属性的属性值，如果属性name存在，则直接返回其属性值；如果属性name不存在，则触发AttribetError异常或当可选参数default定义时返回default值
        self._box_coder = getattr(boxCoders, box_coder)()
        self._box_code_size = box_code_size
        self._num_output_filters = num_output_filters
        # 从通道数为num_output_filters的特征卷积出通道数num_anchor_per_loc*num_cls的特征，作为类别预测结果
        # 若num_cls=1，可以说大于0.5就是目标类
        self.conv_cls = nn.Conv2d(num_output_filters, num_anchor_per_loc * self._num_class, 1)
        # 从通道数为num_output_filter的特征卷积出通道数num_anchor_per_loc*box_code_size的特征，作为3D框的回归结果
        # 每一个位置上的每一个Anchor都要回归出一个3D框和它的执行度score
        self.conv_box = nn.Conv2d(
            num_output_filters, num_anchor_per_loc * box_code_size, 1)
        # 从通道数为num_output_filter的特征卷积出通道数num_anchor_per_loc*2的特征，作为类别预测结果
        # 每一个位置上的每一个Anchor都要回归出2个方向，即面向相机，还是背对相机
    ...
    def forward(self, x):       # torch.Size([2, 256, 200, 176])
        ...
        box_preds = box_preds.view(N, self._num_class, -1, H, W)   # torch.Size([2, 200, 176, 14])
        cls_preds = cls_preds.view(N, self._num_class, -1, H, W)   # torch.Size([2, 200, 176, 2])

        # [N, C, y(H), x(W)]
        # 计算每个位置每个Anchor的3D框预测结果和置信度
        ...
        if self._use_direction_classifier:
            ...
            dir_cls_preds = dir_cls_preds.permute(0, 1, 3, 4, 2).contiguous() # torch.Size([2, 200, 176, 4])

        return box_preds, cls_preds, dir_cls_preds

    # input
    # box_preds   : torch.Size([2, 1, 200, 176, 14])
    # cls_preds   : torch.Size([2, 1, 200, 176, 2])
    # gt_bboxes   : list:len(gt_bboxes) = B , gt_bboxes[0].shape = torch.Size([num_of_gt_bboxes, 7])，torch.Size([15, 7]) or torch.Size([10, 7])
    # anchors      : dict anchors['Car'].size() torch.Size([2, 70400, 7])
    ...
    def loss(self, box_preds, cls_preds, dir_cls_preds, gt_bboxes, gt_labels, gt_types, anchors, anchors_mask, cfg):
        ...
        # 构建 loss
        # 位置误差：预测值是 box_preds， 真值是 reg_targets，权值是 reg_weights，使用weighted_smoothl1
        # 类别误差：预测值是 cls_preds， 真值是 cls_targets，权值是 cls_weights，使用weighted_sigmoid_focal_loss
...
def gen_sample_grid(box, window_size=(4, 7), grid_offsets=(0, 0), spatial_scale=1.):
    ...
    xg1 = xg.unsqueeze(-1)
    xg = xg1.expand(N, *window_size)
    yg1 = yg.unsqueeze(-1)
    yg = yg1.expand(N, *window_size)
    rg1 = rg.unsqueeze(-1)
    rg = rg1.expand(N, *window_size)
...
class PSWarpHead(nn.Module):
    # 根据cfg文件，grid_offsets=（0.， 40.），featmap_stride=0.4,
    # in_channel=256, num_parts=28, num_class=1
    ...
    def forward(self, x, guided_anchors, is_test=False):
        ...
        # 如果是推断阶段，还会把guided_anchors留下来，后续还会使用
        if is_test:
            return bbox_scores
        else:
